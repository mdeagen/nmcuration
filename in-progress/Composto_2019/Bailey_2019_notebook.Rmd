...
mydf <- add_column(mydf, diffusivity = as.list(NA)) # 2020-06-04 added as.list() for nested tibble handling
...
for (i in 1:nrow(temp_diff)) {
  mydf <- add_row(mydf, NP_loading=temp3[i,1],matrix_Mw=100000,diffusivity=as.list(as.character(tibble(temp_diff)[i,]))) # 2020-06-04 added as.character
}
...
for (i in 1:nrow(temp_visc)) {
  mydf <- add_row(mydf, NP_loading=0.05,matrix_Mw=temp5[i,1],viscosity=as.list(tibble(temp_visc)[i,])) # 2020-06-04 added as.list
}
...
for (i in 1:nrow(temp_Tg)) {
  mydf <- add_row(mydf, NP_loading=temp7[i,2]/100,matrix_Mw=temp7[i,1],Tg=as.list(tibble(temp_Tg)[i,])) # 2020-06-04 added as.list
}
