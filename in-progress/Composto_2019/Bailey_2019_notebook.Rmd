...
data_path <- "./RawDataFromAuthors/1911_BaileyEric_OAPS_P2VP_Dynamics_Data_V1.xlsx" # added sub-directory 2020-06-04
...
mydf <- add_column(mydf, diffusivity = list(NA)) # 2020-06-04 added list() for nested tibble handling
...
for (i in 1:nrow(temp_diff)) {
  mydf <- add_row(mydf, NP_loading=temp3[i,1],matrix_Mw=100000,diffusivity=list(tibble(temp_diff)[i,])) # 2020-06-04 added as.character
}
...
mydf <- add_column(mydf, viscosity = list(NA))
mydf$viscosity <- as.list(mydf$viscosity)
temp_visc <- tibble(a=as.character(temp5[,4]),b="Pa*s") # 2020-06-04 added as.character
for (i in 1:nrow(temp_visc)) {
  mydf <- add_row(mydf, NP_loading=0.05,matrix_Mw=temp5[i,1],viscosity=list(tibble(temp_visc)[i,])) # 2020-06-04 added as.list
}
...
mydf <- add_column(mydf, Tg = list(NA))
mydf$Tg <- as.list(mydf$Tg)
temp_Tg <- tibble(a=as.character(temp7[,3]),b="Celsius") # 2020-06-04 added as.character
for (i in 1:nrow(temp_Tg)) {
  mydf <- add_row(mydf, NP_loading=temp7[i,2]/100,matrix_Mw=temp7[i,1],Tg=list(tibble(temp_Tg)[i,])) # 2020-06-04 added as.list
}
...
for (i in 1:4) {
  ...
  mydf <- add_row(mydf, NP_loading=c(0,0.07,0.15,0.25)[i], matrix_Mw=100000, Xray_filename="_XrayScattering_data.xlsx", Xray_df=paste0("D8.",i)) # 2020-06-04 changed from .csv to .xlsx
}
...
for (i in 1:4) {
  ...
  colnames(temp) <- c("Frequency (Hz)", "NormalizedPermittivity_ImaginaryComponent") # note special characters in column 2 name
  assign(paste0("D10.",i), temp)
  
  # add filename and datafile to appropriate rows in dataframe
  mydf <- add_row(mydf, NP_loading=c(0,0.07,0.15,0.25)[i], matrix_Mw=100000, BDS_filename="_BDS_data.xlsx", BDS_df=paste0("D10.",i)) # 2020-06-04 changed from .csv to .xlsx
}
...
for (i in 1:3) {
  ...
  # add filename and datafile to appropriate rows in dataframe
  mydf <- add_row(mydf, NP_loading=0, matrix_Mw=c(33000,100000,467000)[i], BDS_filename="_BDS_data.xlsx", BDS_df=paste0("D12.",i)) # 2020-06-04 changed from .csv to .xlsx
}
```
...
for (j in 1:ncol(mydf)) {
  if(grepl("_datafile",names(mydf)[j])){
    for (i in 1:nrow(mydf)) {
      if(!is.na(mydf[i,j-1])) mydf[[i,j]] <- list(get(mydf[[i,j-1]]))
    }
  }
}
...
mappings <- tibble(variable = colnames(mydf), sheet = as.character(NA), cellrow = as.numeric(NA), cellcol = as.numeric(NA)) # 2020-06-04 added as.character, as.numeric
...
dir.create("./SUBMISSION", showWarnings = FALSE) # 2020-06-04 changed showWarnings to FALSE
...
for (i in 1:nrow(mydf)) {
  ...
  dir.create(sample_folder, showWarnings = FALSE) # create directory (if already exists, will throw warning) # 2020-06-04 changed showWarnings to FALSE
  ...
  for (j in 1:ncol(mydf)) {
    ...
    if(grepl("_datafile", names(mydf)[j])) { # only execute if "_datafile" is in column header
      ...
      if(!is.na(mydf[[i,j-1]])) { # execute if the adjacent df cell NOT NA
        # write nested tibble in _datafile column to csv in sub-directory (COMMENTED OUT 2020-06-04)
        ...
        # updated 2020-06-04
        # write supplementary datafiles to .xlsx
        if(!file.exists(paste0(sample_folder, "/", mydf[[i,j-2]]))) # due to redundancy in mydf, only create if does not exist (added 2020-06-04)
           writeWorksheetToFile(file=paste0(sample_folder, "/", mydf[[i,j-2]]), data=get(mydf[[i,j-1]]), sheet="Sheet1")
      }
    }
  }
}
...
# 2020-06-04: Only populate filler info if NP_loading > 0

# FILLER INFO 
fillerinfo <- tribble(
  ~x,~y,~z,
  "octa(aminophenyl) polyhedral oligomeric silsesquioxane",NA,NA, # Filler chemical name/filler name
  NA,NA,NA, # Filler PubChem ref
  "OAPS",NA,NA, # Filler Abbrev
  NA,NA,NA, # Mfgr or source name
  NA,NA,NA, # Trade name
  NA,NA,NA, # Density
  NA,NA,NA, # Crystal phase
  NA,"1.8","nm", # Particle diameter
)
  
# Add FILLER INFO only if NP_loading > 0 (use uniquedf to accomplish this without unnecessary repetion)
for(i in 1:nrow(uniquedf)) {
  if(uniquedf$NP_loading[i]>0){
    writeWorksheetToFile(file=paste0("./SUBMISSION/", uniquedf$sampleID[i], "/", uniquedf$sampleID[i], "_template.xlsx"),
                         data = fillerinfo, sheet = "2. Material Types", startRow = 28, startCol = 2, header=FALSE)
  }
}
```
