...
image_path <- "./TEM-20191212T195835Z-001/TEM"
...
fig4all <- tibble(graftdens=NA,NP_loading=NA,SAXS_df=NA)
...
fig10all <- tibble(graftdens=NA,NP_loading=NA,Tiso=NA,SAXS_df=NA)
...
mydf$SAXS_filename <- ifelse(!is.na(mydf$SAXS_df),paste0(mydf$sampleID,"_SAXS_data.csv"),NA)
...
mappings <- tibble(variable = colnames(mydf), sheet = NA, cellrow = NA, cellcol = NA, unit = NA)
...
### **Auto-Filling Excel Templates** and **Exporting Supplementary Datafiles (.csv)**
